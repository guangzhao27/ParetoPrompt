...
def perplexity_calculate(input_text, model_name_p, model_p, tokenizer_p):
    ...
    if model_name_p == "textattack/roberta-base-CoLA":
        ...
        # Tokenize input
        inputs = tokenizer_p(sentence, padding=True, truncation=True, return_tensors="pt")
        with torch.no_grad():
        ...
        acceptability_score = probabilities[:, 1]
        return torch.tensor(acceptability_score)
    
    elif model_name_p == "gpt2":
        input_ids = tokenizer_p.encode(input_text, padding=True, truncation=True, return_tensors="pt")
...
class PromptedTextStyleTransferReward(BaseReward):
    def __init__(
        ...
        assert objective_type in ['two', 'three1', 'three2', 'four']
        # objective three includes fluency, four includes fluency and conciseness
        self.objective_type = objective_type
        
        if self.objective_type in ['three1', 'three2', 'four']:
    ...
    def forward(
        ...
        for i, (prompt, src, label) in enumerate(zip(prompt_strs,
            ...
            elif self.objective_type == 'three2':
                # corrected_string = " ".join([token.replace('Ġ', '') for token in prompt_tokens[i]])
                perplexity_scores = perplexity_calculate(hypos, self.model_name_p, self.model_p, self.tokenizer_p)
                perplexity = torch.tensor(perplexity_scores).float().mean()
                perplexities.append(perplexity)
            elif self.objective_type == 'four':
                perplexity_scores = perplexity_calculate(hypos, self.model_name_p, self.model_p, self.tokenizer_p)
                perplexity = torch.tensor(perplexity_scores).float().mean()
                perplexities.append(perplexity)
                
                length_scores = [len(self.tokenizer_p.tokenize(sentence)) for sentence in hypos]
                length = torch.tensor(length_scores).float().mean()
                lengths.append(length)
        
        
        if self.objective_type in ['three1', 'three2', 'four']:
            perplexity_tensor = torch.stack(perplexities)
            if self.objective_type in ['four']:
                neg_length_tensor = 100 - torch.stack(lengths)
        ...
        if self.objective_type in ['four']:
            rewards_tensor = rewards_tensor+neg_length_tensor
            multi_rewards_dict['conciseness'] = neg_length_tensor
